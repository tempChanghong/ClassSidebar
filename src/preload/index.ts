...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    // 保存配置信息
    saveConfig: (newConfig: any): Promise<void> => ipcRenderer.invoke('save-config', newConfig),

    // 监听配置更新
    onConfigUpdated: (callback: (newConfig: any) => void): void => {
        ipcRenderer.on('config-updated', (_event: IpcRendererEvent, newConfig: any) => callback(newConfig))
    },

    // 添加快捷方式
    addShortcut: (filePath: string): Promise<any> => ipcRenderer.invoke('add-shortcut', filePath),

    // 打开文件选择对话框
    openFileDialog: (): Promise<any> => ipcRenderer.invoke('open-file-dialog'),
    ...
    // 获取文件图标
    getFileIcon: (path: string): Promise<string> => ipcRenderer.invoke('get-file-icon', path),
    ...
    // 获取开机自启设置
    getLoginItemSettings: (): Promise<any> => ipcRenderer.invoke('get-login-item-settings'),

    // 设置开机自启
    setLoginItemSettings: (settings: any): Promise<void> => ipcRenderer.invoke('set-login-item-settings', settings),
