...
// 常量定义
const START_W = 4
let START_H = 64 // 会从配置更新
let TARGET_W = 400 // 会从配置更新
const TARGET_H = 450
const THRESHOLD = 60
...
function updateWindowSize(p: number) {
  ...
  if (p <= 0) {
    ...
    // setIgnoreMouse(false) // Vue 中可能需要另外处理
  } else {
...
function expand() {
  ...
  if (store.isExpanded && !store.isDragging && !animationId && Math.abs(currentW - TARGET_W) < 1) return

  stopAnimation()
  store.setExpanded(true)
  document.body.classList.add('expanded') // 保持与 CSS 兼容
  ...
  function animate(currentTime: number) {
    if (!store.isExpanded) { animationId = null; return }
...
const onBlur = () => { if (store.isExpanded) collapse() }
